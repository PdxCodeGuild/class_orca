##############################################################################
# John Fial, PDX Code Guild, 2020-2021
# https://github.com/PdxCodeGuild/class_orca/blob/main/1%20Python/labs/lab21-count_words.md
##############################################################################

# FILENAMES:
# testbook.txt
# carroll_alices_adventures_in_wonderland.txt
# jung_~1920_collected_papers_on_analytical_psychology.txt

import string

# This line makes it slightly simpler to change the filename
file_to_open = 'carroll_alices_adventures_in_wonderland.txt'

# Open file as UTF-9 and save it as a string 'book'
print(f'Welcome to word counter v1.0! Opening file: {file_to_open}...')
with open(file_to_open, 'r', encoding='utf-8') as f:
    book = f.read()
    # print(open('testbook.txt', 'r')) # If you want to verify the encoding type


# Count the number of characters... 
characters = len(book)
# Count the number of words...split at space
words = book.count(' ')
# Count the number of sentences...split at periods
sentences = book.count('.')
print(f'Your file: {file_to_open} contains {characters} characters, {words} words, or {sentences} sentences...')

#     Make everything lowercase, strip punctuation, split into a list of words.
# To strip punctuation, merrit's code:
translator = str.maketrans('', '', string.punctuation) # What does this do?
book_modified = book.translate(translator) # I am a string with punctuation
# lowercase it
book_modified = book_modified.lower()

# Add each word to a word_list
word_list = book_modified.split()
# print(word_list) #Only print for testing -- huge list, but readable!


stop_words = ['i', 'me', 'my', 'said', 'myself', 'we', 'our', 'ours', 'ourselves', 'you', "you're", 
"you've", "you'll", "you'd", 'your', 'yours', 'yourself', 'yourselves', 'he', 'him', 'his', 
'himself', 'she', "she's", 'her', 'hers', 'herself', 'it', "it's", 'its', 'itself', 'they', 'them', 
'their', 'theirs', 'themselves', 'what', 'which', 'who', 'whom', 'this', 'that', "that'll", 'these', 
'those', 'am', 'is', 'are', 'was', 'were', 'be', 'been', 'being', 'have', 'has', 'had', 'having', 
'do', 'does', 'did', 'doing', 'a', 'an', 'the', 'and', 'but', 'if', 'or', 'because', 'as', 'until', 
'while', 'of', 'at', 'by', 'for', 'with', 'about', 'against', 'between', 'into', 'through', 'during', 
'before', 'after', 'above', 'below', 'to', 'from', 'up', 'down', 'in', 'out', 'on', 'off', 'over', 
'under', 'again', 'further', 'then', 'once', 'here', 'there', 'when', 'where', 'why', 'how', 'all', 
'any', 'both', 'each', 'few', 'more', 'most', 'other', 'some', 'such', 'no', 'nor', 'not', 'only', 
'own', 'same', 'so', 'than', 'too', 'very', 's', 't', 'can', 'will', 'just', 'don', "don't", 'should', 
"should've", 'now', 'd', 'll', 'm', 'o', 're', 've', 'y', 'ain', 'aren', "aren't", 'couldn', "couldn't", 
'didn', "didn't", 'doesn', "doesn't", 'hadn', "hadn't", 'hasn', "hasn't", 'haven', "haven't", 'isn', 
"isn't", 'ma', 'mightn', "mightn't", 'mustn', "mustn't", 'needn', "needn't", 'shan', "shan't", 'shouldn', 
"shouldn't", 'wasn', "wasn't", 'weren', "weren't", 'won', "won't", 'wouldn', "wouldn't"]




# Build the word_dictionary
skipped_words = 0
word_dictionary = {}
for word in word_list:

    #But only with words not in the stop list...
    if word not in stop_words:
        if word in word_dictionary:
            # If the word is in the dictionary, pull its current value and replace it with that + 1
            word_dictionary[word] = (word_dictionary[word] + 1)
        else:
            # Add it io the dictionary as a key, with a value of 0
            word_dictionary[word] = 0

    # Otherwise just add to a counter to report later on the skipped words
    else:        
        skipped_words += 1


# SECTION: Print the most frequent top 10 words with their counts.

# Makes the dictionary into a list [], with tuples
word_list = list(word_dictionary.items()) # .items() returns a list of tuples
word_list.sort(key=lambda tup: tup[1], reverse=True)  # sort largest to smallest, based on count

print(f'{skipped_words} instances of words were removed because they were found in the stop list (the, if, and, she, there, etc.).')

# Print the top words!
for i in range(min(10, len(word_list))):  # print the top (min(10, or all of them)) words, whichever is smaller
    tuple = word_list[i]
    print(f'The word \"{tuple[0]}\" was found {tuple[1]} times.')

print('Thank you!')














##############################################################################
# https://github.com/PdxCodeGuild/class_orca/blob/main/1%20Python/labs/lab21-count_words.md
##############################################################################

# Lab 21: Count Words

# Let's write a python module to analyze a given text file containing a book for its vocabulary frequency and display the most frequent words to the user in the terminal. Remember there isn't any "perfect" way to identify a word in english (acronymns, mr/ms, contractions, etc), try to find rules that work best.

# Find a book on Project Gutenberg. Download it as a UTF-8 text file.

#     Open the file.
#     Make everything lowercase, strip punctuation, split into a list of words.

# To strip punctuation:

# import string
# s = 'I $am a !string with punc&^%*tuation!'
# translator = str.maketrans('', '', string.punctuation)
# string_without_punct = s.translate(translator) # I am a string with punctuation

#     Your dictionary will have words as keys and counts as values. If a word isn't in your dictionary yet, add it with a count of 1. If it is, increment its count.
#     Print the most frequent top 10 out with their counts. You can do that with the code below.

# # word_dict is a dictionary where the key is the word and the value is the count
# words = list(word_dict.items()) # .items() returns a list of tuples
# words.sort(key=lambda tup: tup[1], reverse=True)  # sort largest to smallest, based on count
# for i in range(min(10, len(words))):  # print the top 10 words, or all of them, whichever is smaller
#     print(words[i])

# You'll find that the most common words aren't particularly interesting (lots of 'I's, 'the', 'and', 'he', 'she', and 'but's). To get more relevant words, remove stop words from your text.

# Version 2 (optional)

# Count how often each unique pair of words is used, then print the top 10 most common pairs with their counts.
# Version 3 (optional)

# Let the user enter a word, then show the words which most frequently follow it.